...
*/
public class PingPongEngine implements Runnable, MouseMotionListener, KeyListener, PongConstants{
...
char key = e.getKeyChar();
 if ('8' == key && kidRacket_Y>=TABLE_TOP){
	
	kidRacket_Y = kidRacket_Y - RACKET_INCREMENT;

} if ('5' == key && kidRacket_Y<=TABLE_BOTTOM - RACKET_LENGTH){
	kidRacket_Y = kidRacket_Y + RACKET_INCREMENT;
}table.setPlayerRacket(kidRacket_Y);
...
ballY=kidRacket_Y;
if (ballY > 0 && ballY <= TABLE_HEIGHT/4 ){
verticalSlide=1;
}
else if(ballY>TABLE_HEIGHT/4 && ballY<=TABLE_HEIGHT/2){
verticalSlide=-1;
}
else if(ballY>TABLE_HEIGHT/2 && ballY<=TABLE_HEIGHT*3/4){
verticalSlide=1;
}
else{
verticalSlide=-1;
